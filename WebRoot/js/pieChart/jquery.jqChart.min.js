(function(a) {
	a.fn.jqChart = function(b, d, e) {
		if (typeof b === "object")
			...
			if (a.fn.jqChart.methods[b])
				...
				a.error("Method " + method
						+ " does not exist on jQuery.jqChart")
		}
	...
	a.jqChartSprintf = function() {
		...
		function d(a, d, b, c, g) {
			...
			if (f > 0)
				if (b || !g)
					...
					a = a.slice(0, d.length) + e("", f, "0", true)
							+ a.slice(d.length);
			return a
		}
		...
		var b = arguments, f = 0, h = b[f++];
		return h
				.replace(
						a.jqChartSprintf.regex,
						function(t, s, q, a, w, h, m) {
							if (t == "%%")
								return "%";
							for (var j = false, n = "", k = false, l = false, r = 0; q
									&& r < q.length; r++)
								switch (q.charAt(r)) {
								case " ":
									n = " ";
							...
							case "G":
								var o = +i, p = o < 0 ? "-" : n, v = ([
										...
										"toPrecision" ])["efg".indexOf(m
										.toLowerCase())], u = ([ "toString",
										"toUpperCase" ])["eEfFgG".indexOf(m) % 2];
								i = p + Math.abs(o)[v](h);
	...
	a.jqChartDateFormatter = function() {
		var e = /d{1,4}|m{1,4}|yy(?:yy)?|([HhMsTt])\1?|[LloSZ]|"[^"]*"|'[^']*'/g, d = /\b(?:[PMCEA][SDP]T|(?:Pacific|Mountain|Central|Eastern|Atlantic) (?:Standard|Daylight|Prevailing) Time|(?:GMT|UTC)(?:[-+]\d{4})?)\b/g, c = /[^-+\dA-Z]/g, b = function(
				a, b) {
			a = String(a);
			b = b || 2;
			while (a.length < b)
		...
		return function(f, h, l) {
			var k = a.jqChartDateFormat, g = k.amPm;
			...
			var j = l ? "getUTC" : "get", o = f[j + "Date"](), r = f[j + "Day"]
					(), n = f[j + "Month"](), u = f[j + "FullYear"](), i = f[j
					+ "Hours"](), s = f[j + "Minutes"](), t = f[j + "Seconds"]
					(), m = f[j + "Milliseconds"](), p = l ? 0 : f
					.getTimezoneOffset(), q = {
				d : o,
				dd : b(o),
				ddd : k.dayNames[r],
				...
				Z : l ? "UTC" : (String(f).match(d) || [ "" ]).pop().replace(c,
						""),
				o : (p > 0 ? "-" : "+")
						+ b(Math.floor(Math.abs(p) / 60) * 100 + Math.abs(p)
								% 60, 4),
				S : k.s(o)
			};
	...
	a.jqChartDateFormat = {
		...
		amPm : [ "am", "pm", "AM", "PM" ],
		s : function(a) {
			return a < 11 || a > 13 ? ([ "st", "nd", "rd", "th" ])[Math.min(
					(a - 1) % 10, 3)] : "th"
		},
	...
	jb.prototype = {
		...
		animate : function(a) {
			...
			if (a > this.startTime) {
				var c = a.getTime() - this.startTime.getTime(), b = this.from
						+ this.valueDiff * c / this.timeDiff;
				if (b === this.to)
					this.lastIsSet = true;
	...
	e.setShadows = function(d, a, e) {
		...
		d.shadowColor = !b.isNull(a.shadowColor) ? a.shadowColor
				: c.shadowColor;
		d.shadowBlur = !b.isNull(a.shadowBlur) ? a.shadowBlur : c.shadowBlur;
		d.shadowOffsetX = !b.isNull(a.shadowOffsetX) ? a.shadowOffsetX
				: c.shadowOffsetX;
		d.shadowOffsetY = !b.isNull(a.shadowOffsetY) ? a.shadowOffsetY
				: c.shadowOffsetY
	};
	...
	e.checkIfRetinaDisplay = function() {
		...
		return window.devicePixelRatio > 1 ? true : window.matchMedia
				&& window.matchMedia(a).matches ? true : false
	};
	...
	e.prototype = {
		...
		_setOptions : function() {
			...
			!this.tooltip.hasClass(this.tooltipClass)
		...
		_initMouseInput : function(b) {
			...
			var a = this.locOffset, f = c - a.left, g = d - a.top, e = {
				x : c,
				y : d,
				locX : f,
				locY : g
		...
		_initTouchInput : function(j) {
			...
			for (var c = this.locOffset, d = [], a = 0; a < b.length; a++) {
				var e = b[a], f = e.pageX, g = e.pageY, h = f - c.left, i = g
						- c.top;
				d.push({
					x : f,
					y : g,
		...
		_resize : function() {
			var c = this.elem, e = c.width(), d = c.height();
		...
		_processMouseEvents : function() {
			var b = this.mouseInput, a = this.hitTest(b.locX, b.locY);
		...
		_processTooltips : function(c) {
			var d = this.hasTooltips, f = this.hasHighlighting;
			if (!d && !f)
				return;
			var g = this.options.tooltips.snapArea, a = this._getTooltipShapes(
					c.locX, c.locY, g, c), e = true;
			if (this._oldTShapes == null
					|| !b.compareArrays(this._oldTShapes, a)) {
				if (a != null) {
					this._initTooltip(a);
		...
		_setTooltipPos : function(d, g) {
			var k = this.tooltip.outerWidth(true), f = this.tooltip
					.outerHeight(true), h = this._width, e = this._height, j = d[0]
					._getTooltipPosition(g, k, f, h, e), c = j.y, i = d.length, l = this;
			if (i > 1) {
				c = 0;
				a.each(d, function() {
		...
		_initTooltip : function(b) {
			...
			if (g == 1)
				if (c.borderColor) {
					c.borderColor == "auto"
							&& this.tooltip.css("border-color", b[0]
									.getTooltipColor());
		...
		_highlightShapes : function(g) {
			...
			var f = this.options.tooltips, b = [];
			a.each(g, function(d, c) {
				var a = c._createHighlightShape(f.highlightingFillStyle,
						f.highlightingStrokeStyle);
				b.push(a)
			});
			var c;
			if (b.length == 1) {
		...
		_resetCursor : function() {
			this.elem.css("cursor") != this.originalCursor
					&& this.elem.css("cursor", this.originalCursor)
		},
	...
	kb.prototype = {
		_calculateActualInterval : function(m, l) {
			...
			if (!this.getOrientation || this.getOrientation() == "x")
				h = .8;
			for (var k = h * this.maxInter200Px, e = Math.max(this.length * k
					/ 200, 1), g = l - m, a = g / e, j = Math.pow(10, Math
					.floor(b.log10(a))), f = [ 10, 5, 2, 1 ], c = 0; c < f.length; c++) {
				var i = f[c], d = j * i;
				if (e < g / d)
					break;
				a = d
		...
		_setVisibleRanges : function() {
			this.actualVisibleMinimum = b.isNull(this.visibleMinimum) ? this.actualMinimum
					: this.visibleMinimum;
			this.actualVisibleMaximum = b.isNull(this.visibleMaximum) ? this.actualMaximum
					: this.visibleMaximum;
		...
		_setMinMax : function(c, a) {
			if (this.logarithmic) {
				this.actualMinimum = b.isNull(this.minimum) ? c : b.log(
						this.minimum, this.logBase);
				this.actualMaximum = b.isNull(this.maximum) ? a : b.log(
		...
		_getIntervals : function(c, b) {
			...
			if (b && b.intervalOffset)
				d = b.intervalOffset;
			for (var e = [], f = this._getIntervalStart(
					this.actualVisibleMinimum, c), a = f + d; a <= this.actualVisibleMaximum; a = this
					._getNextPosition(a, c))
				e.push(a);
			return e
		},
		_getIntervalStart : function(d, b) {
			var c = d - this.getCrossing(), a = this._alignToInterval(c, b);
			if (a < d)
		...
		_measureRotatedLabels : function(g) {
			for (var h = this.isAxisVertical, c = 0, b = 0, d = 0; d < g.length; d++) {
				var a = g[d], e = Math.sqrt(a.width * a.width + a.height
						* a.height), f = a.rotationAngle;
				if (h) {
		...
		_correctLabelsPositions : function(n) {
			var i = 0, h = 0, o = this.reversed === true, p = this.labels.position == "inside", q = this.isAxisVertical, g = this.lblMargin;
			if (q) {
				for (var e = [], d = 0; d < n.length; d++) {
					var a = n[d], f = false, k = a.y;
		...
		_removeOverlappedLabels : function(c) {
			var f = 0, e = 0, i = 0, o = 0, l = 0, j = 0, n = 0, m = h, s = this.reversed === true, u = this.labels.position == "inside", r = this.isAxisVertical, g = this.lblMargin, k = 2 * g;
			if (r)
				for (var v = [], b = 0; b < c.length; b++) {
					var d = b;
					if (this.reversed)
						d = c.length - b - 1;
					var a = c[d], t = false, q = a.y;
					switch (a.textBaseline) {
					case "middle":
						q -= a.height / 2;
						break;
		...
		_arrange : function() {
			var a = this.x, d = this.y, c = this.x + this.width, e = this.y
					+ this.height;
			...
			if (this.title.text)
				...
				case "left":
					...
					this.title.rotY = this.y + (this.height + this.title.width)
							/ 2;
		...
		_render : function() {
			var b = [], e = this._getTickMarks(this.minorTickMarks, false);
			a.merge(b, e);
		...
		_getMainLine : function() {
			var b = this.crossOffsetX, c = this.crossOffsetY, a = new n(this.x1
					+ b, this.y1 + c, this.x2 + b, this.y2 + c);
		...
		_getMaxInsideTickMarksLength : function() {
			var a = 0;
			if (this.minorTickMarks != null && this.minorTickMarks.visible
					&& this.minorTickMarks.isInside())
				a = Math.max(a, this.minorTickMarks.length);
			if (this.majorTickMarks != null && this.majorTickMarks.visible
		...
		_getLabels : function() {
			...
			if (c == null || c.visible === false)
				return [];
			var g = c.position == "inside", e = this.lblMargin, l = this.offset, i = this.crossOffsetX, j = this.crossOffsetY, n = this.isAxisVertical;
			if (n && c.vAlign == "center" || !n && c.hAlign == "center")
				e += g ? this._getMaxInsideTickMarksLength() + this.lineWidth
						/ 2 : this._getMaxOutsideTickMarksLength();
			var o = [], q = this._getMarkInterval(c, true);
			if (!q)
				return [];
			for (var p = this._getLabelIntervals(q, c), r = p.length, t = c.showFirstLabel, u = c.showLastLabel, h = 0; h < r; h++) {
				if (!t && h == 0 || !u && h == r - 1)
					continue;
				var s = p[h], v = this.getLabel(s), a = this._createLabel(v, c), m = this
						.getPosition(s);
				switch (this.location) {
				case "left":
					if (g)
					...
					if (this.labels.angle) {
						var d = Math.min(90, Math.max(-90, this.labels.angle)), f = b
								.radians(d);
						a.rotX = a.x;
						a.rotY = a.y;
						a.rotationAngle = f
				...
				case "top":
					...
					if (this.labels.angle) {
						...
						if (d > 0)
							d = -180 + d;
						var f = b.radians(d), k = Math.sqrt(a.width * a.width
								+ a.height * a.height);
		...
		_getTickMarks : function(c, p) {
			if (c == null || c.visible != true)
				return [];
			for (var m = [], g = this.crossOffsetX, h = this.crossOffsetY, j = this.offset, i = c.position == "inside", r = this
					._getMarkInterval(c, p), b = c.length, o = this
					._getIntervals(r, c, p), d, f, e, a, l = 0; l < o.length; l++) {
				...
				case "top":
					d = e = k;
					if (i)
						a = this.y + this.height + b + this.lineWidth / 2 + h;
					else
						a = this.y + this.height - j + h;
					f = a - b;
					break;
				case "bottom":
					d = e = k;
					if (i)
						a = this.y - b - this.lineWidth / 2 + h;
					else
						a = this.y + j + h;
					f = a + b
				}
		...
		getZoom : function() {
			if (!this.actualMaximum)
				return 1;
			var b = this.actualMaximum - this.actualMinimum, a = this.actualVisibleMaximum
					- this.actualVisibleMinimum, c = a / b;
			return c
		},
		setOptions : function(c) {
			...
			if (b.majorGridLines)
				this.majorGridLines = new Gb(b.majorGridLines);
			if (b.minorGridLines) {
		...
		getPosition : function(f) {
			var e = this.actualVisibleMaximum, d = this.actualVisibleMinimum, a = this.length
					/ (e - d) * (f - d), b = this.reversed === true, c = this.isAxisVertical;
		...
		getValue : function(f) {
			var e = this.actualVisibleMaximum, a = this.actualVisibleMinimum, b = (f - this.origin)
					* (e - a) / this.length + a, c = this.reversed === true, d = this.isAxisVertical;
		...
		isVertical : function() {
			return this.location == "left" || this.location == "right" ? true
					: false
		},
		isValueVisible : function(a) {
			if (this.logarithmic)
				a = b.log(a, this.logBase);
			return a >= this.actualVisibleMinimum
					&& a <= this.actualVisibleMaximum
		},
		isInVisibleRange : function(d) {
			var c = this.visibleMinimum, a = this.visibleMaximum;
	...
	var k = -Number.MAX_VALUE, h = Number.MAX_VALUE;
	function b() {
	}
	b.isNull = function(a) {
	...
	b.round = function(a) {
		...
		var e = a.toPrecision(14), f = parseFloat(e);
		return f
	};
	b.log10 = function(a) {
	...
	b.rotatePointAt = function(j, k, c, a, b) {
		var e = Math.sin(c), d = Math.cos(c), f = j - a, g = k - b, h = a + f
				* d - g * e, i = b + f * e + g * d;
		return {
			x : h,
			y : i
	...
	d.getDaysInMonth = function(b, a) {
		return a == 1 ? (new Date(b, 1, 29)).getDate() == 29 ? 29 : 28 : ([ 31,
				undefined, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31 ])[a]
	};
	...
	d.addMonths = function(c, b) {
		var a = new Date(c.getTime()), e = a.getDate();
		a.setDate(1);
	...
	c.compare = function(b, e, a, j) {
		...
		if (b.hitTest(a.locX, a.locY, 0))
			return true;
		var f = b.getCenter(a), g = e.getCenter(a), c = a.locX - f.x, d = a.locY
				- f.y, h = Math.sqrt(c * c + d * d);
		c = a.locX - g.x;
		d = a.locY - g.y;
	...
	c.prototype.boundsHitTest = function(b, c, a) {
		if (!this.useHitTestArea)
			a = 0;
		return b >= this.x - a && b <= this.x + this.width + a
				&& c >= this.y - a && c <= this.y + this.height + a
	};
	...
	c.prototype.renderDashedLine = function(f, h, g, i, l, c) {
		...
		c.moveTo(f, h);
		var a = f, b = h, k = 0, j = true, s = l.length;
		while (!(d.thereYet(a, g) && e.thereYet(b, i))) {
			var o = Math.atan2(i - h, g - f), p = l[k];
	...
	c.prototype._createGradient = function(h, c) {
		if (c == null || typeof c == "string" || this.width == null
				|| this.height == null || this.x == null || this.y == null)
			return c;
		var d, b = {
			x0 : 0,
			y0 : 0,
		...
		switch (c.type) {
		case "radialGradient":
			var k = this.x + b.x0 * this.width, m = this.y + b.y0 * this.height, i = b.r0
					* this.width / 2, l = this.x + b.x1 * this.width, n = this.y
					+ b.y1 * this.height, j = b.r1 * this.width / 2;
			d = h.createRadialGradient(k, m, i, l, n, j);
			break;
		default:
			var q = this.x + b.x0 * this.width, r = this.y + b.y0 * this.height, p = this.x
					+ b.x1 * this.width, o = this.y + b.y1 * this.height;
	...
	c.prototype._getAnimationPoints = function(c, a) {
		...
		var f = c.slice(0, a);
		d /= 2;
		var i = c[a - 2], g = c[a - 1], h = c[a], e = c[a + 1];
	...
	n.prototype.hitTest = function(f, g, j) {
		var b = this.x1, d = this.y1, c = this.x2, e = this.y2, a = Math.max(j,
				Math.max(3, this.lineWidth / 2));
		if (b == c) {
			var h = f + 1;
			return h > b - a && h < c + a && g >= d - a && g <= e + a ? true
					: false
		}
		if (d == e) {
			var i = g + .5;
			return f >= b - a && f <= c + a && i > d - a && i < e + a ? true
					: false
		}
		return false
	};
	n.prototype.hitTestNonHV = function(k, l, j) {
		var a = this.x1, b = this.y1, c = this.x2, d = this.y2;
		if (a < c) {
			this.x = a;
			this.width = c - a
		} else {
		...
		var n = Math.sqrt(e * e + f * f), m = (e * k - f * l + i) / n, o = Math
				.max(j, 3);
	...
	n.prototype.render = function(a, i) {
		...
		var e = d(this.x1), g = d(this.y1), f = d(this.x2), h = d(this.y2);
	...
	i.prototype.render = function(d, o) {
		...
		var k = this.strokeStyle != null && this.lineWidth > 0, n = k
				&& Math.floor(this.lineWidth % 2) ? b.roundH : Math.round, m = this.correction || 0;
		m = Math.round(m);
		var h = n(this.x), j = n(this.y), f = Math.round(this.width), e = Math
				.round(this.height), a = this.context;
		if (this.xDecrease) {
			var l = a.series.realYAxis.getCrossing(), g = Math
					.round(this.xDecrease);
			f -= g;
	...
	i.prototype.renderRoundedRectPath = function(b, c, d, g, f) {
		var a = this.cornerRadius, e = Math.PI / 2;
		b.beginPath();
		b.moveTo(c + a, d);
		b.lineTo(c + g - a, d);
	...
	Ib.prototype.hitTest = function(f, g, a) {
		if (this.boundsHitTest(f, g, a) == false)
			return false;
		var c = (this.width + a) / 2, b = (this.height + a) / 2, h = this.x + c, i = this.y
				+ b, d = f - h, e = g - i, j = d * d / (c * c), k = e * e
				/ (b * b);
		return j + k <= 1
	};
	Ib.prototype.render = function(a, m) {
		...
		var d = this.x, f = this.y, l = this.width, k = this.height, g = this.width / 2 * .5522848, h = this.height / 2 * .5522848, i = d
				+ l, j = f + k, e = d + l / 2, b = f + k / 2;
		a.beginPath();
		a.moveTo(d, b);
	...
	Jb.prototype.render = function(a, d) {
		...
		a.arc(Math.round(this.x + b), Math.round(this.y + b), Math.round(b),
	...
	bb.prototype.hitTest = function(e, f, a) {
		if (!this.useHitTestArea)
			a = 0;
		var c = this.x + this.width / 2, d = this.y + this.height / 2, b = Math
				.pow(e - c, 2)
				+ Math.pow(f - d, 2);
	...
	bb.prototype.render = function(a, d) {
		...
		a.beginPath();
		var b = Math.max(0, this.radius);
		a.arc(Math.round(this.x + b), Math.round(this.y + b), Math.round(b), 0,
				Math.PI * 2, false);
		a.closePath();
		this.fillStyle != null && a.fill();
		this.strokeStyle != null && this.lineWidth > 0 && a.stroke()
	...
	function qb(f, g, a, b, d) {
		...
		this.endAngle = d;
		this.width = this.height = 2 * a;
		var e = (b + d) / 2, h = f + a * Math.cos(e), i = g + a * Math.sin(e);
	...
	qb.prototype.hitTest = function(d, e) {
		var b = this.x, c = this.y, f = Math.pow(d - b, 2) + Math.pow(e - c, 2);
		if (f > Math.pow(this.radius, 2))
			return false;
		var g = b - d, h = c - e, a = Math.atan2(h, g) + Math.PI;
		if (a > 3 * Math.PI / 2)
			a -= 2 * Math.PI;
	...
	qb.prototype.render = function(a, i) {
		...
		c.prototype.render.call(this, a, i);
		a.beginPath();
		var g = Math.round(this.x), h = Math.round(this.y), b = this.startAngle, d = this.endAngle;
		if (e.use_excanvas) {
	...
	u.prototype.createHighlightMark = function(c) {
		if (c == -1)
			return null;
		var d = this.context, f = this.pts, g = d.points[c / 2], e = 5, b = new bb(
				f[c] - e, f[c + 1] - e, e);
	...
	v.prototype.renderPoints = function(f, i) {
		...
		if (h) {
			...
			b.unshift(b[e - 1]);
			for (var c = 0; c < e; c += 2)
				d = d.concat(this.getControlPoints(b[c], b[c + 1], b[c + 2],
						b[c + 3], b[c + 4], b[c + 5], g));
			d = d.concat(d[0], d[1]);
			for (var c = 2; c < e + 2; c += 2)
				f.bezierCurveTo(d[2 * c - 2], d[2 * c - 1], d[2 * c],
						d[2 * c + 1], b[c + 2], b[c + 3])
		} else {
	...
	v.prototype.getControlPoints = function(d, f, a, b, e, g, j) {
		var h = Math.sqrt(Math.pow(a - d, 2) + Math.pow(b - f, 2)), k = Math
				.sqrt(Math.pow(e - a, 2) + Math.pow(g - b, 2)), c = j * h
				/ (h + k), i = j - c, l = a + c * (d - e), m = b + c * (f - g), n = a
				- i * (d - e), o = b - i * (f - g);
		return [ l, m, n, o ]
	};
	function cb(b, c, e, f) {
	...
	Q.prototype.hitTest = function(j, a, h) {
		var k = this.context;
		if (k && k.points)
			return u.prototype.hitTest.call(this, j, a, h);
	...
	q.prototype.render = function(a, f) {
		if (!this.visible)
			return;
		c.prototype.render.call(this, a, f);
		var d = b.roundH(this.x), e = b.roundH(this.y);
	...
	q.prototype.renderBg = function(a) {
		...
		if (!e)
			return;
		var g = b.roundH(this.x), h = b.roundH(this.y);
		!this.width && this.measure(a);
		var d = Math.round(this.width), c = Math.round(this.height);
	...
	q.prototype.isInRect = function(e, f, h, g) {
		var a = this.x, b = this.y, d = this.width, c = this.height;
		switch (this.textAlign) {
		case "center":
			a -= d / 2;
			break;
	...
	q.prototype.hitTest = function(a, b, e) {
		if (!this.isLegendItem && !this.isAxisLabel)
			return false;
		var d = this.width, c = this.height;
		switch (this.textAlign) {
		case "center":
			a += d / 2;
			break;
	...
	gb.prototype.render = function(e) {
		if (!this.visible)
			return;
		var b = new Image, c = this.x, d = this.y, a = this;
		b.onload = function() {
			var g = b.width, f = b.height;
			c -= g / 2;
			d -= f / 2;
			a.x = c;
			a.y = d;
	...
	P.prototype._render = function(g) {
		var e = this.offsetX || this.offsetY, f = this.chart.options, c = this.ctx;
		...
		for (var d = 0; d < g.length; d++) {
			var a = g[d];
			if (a) {
				!this.isHighlighting && a.context && a.context.series
						&& !a.isLegendItem && !(a instanceof q)
						&& this.chart.elem.trigger("shapeRendering", a);
	...
	P.prototype._renderShadows = function(h) {
		if (e.use_excanvas)
			return;
		var g = this.offsetX && this.offsetY, c = this.chart.options;
		...
		for (var f = 0; f < h.length; f++) {
			var d = h[f];
			d && d.shadowColor && d.shadowColor != "rgba(0, 0, 0, 0)"
					&& d.render(a)
		}
		g && a.restore()
	};
	...
	Fb.prototype._render = function(f) {
		...
		if (this.text == null || !c)
			return;
		var e = c._width, d = c._height, a = this.margin + this.textBlock.width
				/ 2, b = this.margin;
		switch (this.hAlign) {
		case "center":
			a += (e - this.width) / 2;
	...
	lb.prototype._render = function(c) {
		var a = this.visible ? this.lineWidth / 2 : 0, f = this.x + a, g = this.y
				+ a, e = this.width - 2 * a, d = this.height - 2 * a, b = new i(
				f, g, e, d);
	...
	lb.prototype.getPadding = function() {
		return !this.visible ? 0 : this.lineWidth + this.cornerRadius / 2
				+ this.padding
	};
	function Gb(a) {
		this.defaults = {
	...
	W.prototype.start = function() {
		...
		var b = this.view.originalCursor = a.browser && a.browser.mozilla ? "-moz-grabbing"
				: "move";
		this.view.elem.css("cursor", b)
	};
	W.prototype.mouseMove = function() {
		for (var f = this.zoomableAxes, a = this.view.mouseInput, c = this.zoomableAxes, d = this.oldMouseInput.locX
				- a.locX, e = this.oldMouseInput.locY - a.locY, b = 0; b < c.length; b++)
	...
	N.prototype.doPan = function() {
		for (var e = this.zoomableAxes, c = this.view.touchInput, d = c[0], b = this.oldTouchInput[0], f = b.locX
				- d.locX, g = b.locY - d.locY, a = 0; a < e.length; a++)
	...
	N.prototype.doScale = function() {
		...
		for (var d = this.getTwoTouchPointData(this.oldTouchInput), e = this
				.getTwoTouchPointData(a), c = this.zoomableAxes, b = 0; b < c.length; b++)
	...
	g.prototype._setOptions = function(h) {
		...
		c.crosshairs = a.extend(true, {}, a.fn.jqChart.defaults.crosshairs,
				c.crosshairs);
		c.tooltips = a.extend(true, {}, a.fn.jqChart.defaults.tooltips,
				c.tooltips);
		c.shadows = a
				.extend(true, {}, a.fn.jqChart.defaults.shadows, c.shadows);
		c.selectionRect = a.extend(true, {},
				a.fn.jqChart.defaults.selectionRect, c.selectionRect);
		c.watermark = a.extend(true, {}, a.fn.jqChart.defaults.watermark,
				c.watermark);
		c.toolbar = a
				.extend(true, {}, a.fn.jqChart.defaults.toolbar, c.toolbar);
		c.globalAlpha = b.isNull(c.globalAlpha) ? a.fn.jqChart.defaults.globalAlpha
				: c.globalAlpha;
		c.mouseInteractionMode = c.mouseInteractionMode
				|| a.fn.jqChart.defaults.mouseInteractionMode;
		...
		var g = this._width = this.elem.width(), f = this._height = this.elem
				.height(), c = this.options;
	...
	g.prototype._arrange = function() {
		var f = this._width, g = this._height;
		this.border.width = f;
		this.border.height = g;
		var c = this.border.getPadding();
		f -= 2 * c;
		g -= 2 * c;
		var d = c, e = c + this.title.height, o = this.axes._getTotalWidth(), n = this.axes
				._getTotalHeight();
		if (this.legend._isHorizontal()) {
			this.gridArea.width = Math.round(f - o);
			this.gridArea.height = Math.round(g
					- (n + this.title.height + this.legend.height))
		} else {
		...
		for (var l = this.axes.verCrossingAxis, k = this.axes.horCrossingAxis, i = this.axes
				._getAxesInLoc("left"), b = i.length - 1; b >= 0; b--) {
			var a = i[b];
			a.x = d;
		...
		case "left":
			this.legend.x = c;
			this.legend.y = c + this.gridArea.y
					+ (this.gridArea.height - this.legend.height) / 2;
			break;
		case "top":
			this.legend.x = c + (f - this.legend.width) / 2;
	...
	g.prototype._getClosestShapeAtX = function(f, g) {
		for (var c = null, j = h, k = g.locX, d = f.length - 1; d >= 0; d--) {
			...
			if (e.tooltips && e.tooltips.disabled)
				continue;
			var b = a.getCenter(g), i = Math.abs(b.x - k);
			if (j > i) {
				j = i;
				c = a;
				if (b.mark)
	...
	g.prototype._getClosestShapeAtY = function(c, d) {
		for (var b = null, g = h, j = d.locY, a = c.length - 1; a >= 0; a--) {
			var e = c[a], i = e.getCenter(d).y, f = Math.abs(i - j);
			if (g > f) {
				g = f;
				b = e
			}
	...
	g.prototype._getShapesAtX = function(g, c, e, f) {
		var b = [ c ], d = [ c.context.series ];
		a.each(e, function() {
			if (this != c && this.context && !this.isLegendItem
					&& !this.isAxisLabel) {
				var h = this.context.series, e = this.getCenter(f);
	...
	g.prototype._getTooltipShapes = function(h, i, g, c) {
		var b = null, f = this.options.tooltips;
		if (f.type == "shared") {
			var d = this._getClosestShapeAtX(this.shapes, c);
			if (d)
				b = this._getShapesAtX(d.getCenter(c).x, d, this.shapes, c)
		} else
			b = e.prototype._getTooltipShapes.call(this, h, i, g, c);
		if (b) {
			series = this.series.items;
			b.sort(function(d, e) {
				var b = a.inArray(d.context.series, series), c = a.inArray(
						e.context.series, series);
				return b - c
			})
		}
		return b
	...
	g.prototype._initCrosshairs = function(a) {
		if (!this.hasCrosshairs)
			return;
		var f = this.options.crosshairs, d = a.locX, e = a.locY;
	...
	g.prototype._renderCrosshairs = function(e, f) {
		...
		if (!c._contains(e, f))
			return;
		var b = this.options.crosshairs, d = [];
	...
	g.prototype._renderSelectionRect = function(g, h) {
		var b = this.gridArea, c = b.fitHor(g.locX), e = b.fitVer(g.locY), d = b
				.fitHor(h.locX), f = b.fitVer(h.locY);
		if (!this.canZoomVer) {
			e = b.y;
			f = b.y + b.height
		...
		var m = Math.min(c, d), n = Math.min(e, f), l = Math.abs(c - d), k = Math
				.abs(e - f), j = new i(m, n, l, k);
	...
	g.prototype._createShapes = function() {
		var b = {}, h = this.gridArea, c = b.shapes = [];
	...
	g.prototype._renderShapes = function() {
		var i = e.isRetinaDisplay, d = this._width, c = this._height;
		if (i) {
		...
		this.shapeRenderer._clear();
		e.use_excanvas && this.areaRenderer._clear();
		var j = this.gridArea, b = this.allShapes;
		this.shapeRenderer._render(b.shapes);
		var h = b.gaShapes;
		if (e.use_excanvas)
			this.areaRenderer._render(h);
		else {
			this.ctx.save();
	...
	g.prototype.stringFormat = function(b, c) {
		return a.type(b) == "date" ? a.jqChartDateFormatter(b, c) : a
				.jqChartSprintf(c, b)
	};
	...
	M.prototype.getTooltipColor = function() {
		...
		return this.priceDownStrokeStyle ? c
				.getColorFromFillStyle(this.priceDownStrokeStyle)
				: c.prototype.getTooltipColor.call(this)
	};
	...
	fb.prototype.createElements = function(b, e, f, o, l, m) {
		var d = [], g = Math.floor(m / 2), b = Math.round(b), a = Math.round(o), c = Math
				.round(l);
		if (a > c) {
			var k = c;
			c = a;
	...
	fb.prototype.getTooltipColor = function() {
		return this.isUp ? c.getColorFromFillStyle(this.priceUpFillStyle) : c
				.getColorFromFillStyle(this.priceDownFillStyle)
	};
	function Kb(a) {
		this.colorsDefault = [ "#418CF0", "#FCB441", "#E0400A", "#056492",
				"#BFBFBF", "#1A3B69", "#FFE382", "#129CDD", "#CA6B4B",
				"#005CDB", "#F3D288", "#506381", "#F1B9A8", "#E0830A",
				"#7893BE" ];
	...
	Kb.prototype.getColor = function(b) {
		var a = this.getColors(this.type), c = a.length;
		b %= c;
		return a[b]
	};
	...
	Nb.prototype = {
		_init : function() {
			var b = this.toolbar, c = this.chart;
			if (this.visibility == "hidden" || this.visibility == "auto"
					&& !c.canZoom) {
				if (b) {
					b.remove();
					this.toolbar = null
			...
			var e = b.outerWidth(), h = b.outerHeight(), d = c.gridArea, f = d.x
					+ d.width - e, g = d.y;
			b.css({
				left : f,
				top : g
		...
		_addButtons : function() {
			var f = this.toolbar, d = this.chart, e = a("<li class='ui-corner-all ui-widget-header'><span class='ui-icon ui-icon-arrow-4-diag'></span></li>");
			f.append(e);
			this._addHover(e);
	...
	mb.prototype._isHorizontal = function() {
		return this.location == "top" || this.location == "bottom" ? true
				: false
	};
	...
	mb.prototype._measure = function() {
		...
		this.title._measure();
		var i = this.title.width, f = this.title.height;
		if (this.title.text)
			f += this.padding;
		...
		var h = 2 * this.margin + 2 * this.padding, k = this.chart._width - 2
				* this.chart.border.getPadding() - h;
		if (j && b > k) {
			var e = Math.floor(k / d) || 1, l = Math
					.ceil(this.items.length / e);
			this.rows = e;
	...
	mb.prototype._arrange = function() {
		if (this.visible == false)
			return;
		var d = this.x + this.margin, b = this.y + this.margin, g = 2 * this.margin;
		this.border.x = d;
		this.border.y = b;
		var f = this.width - g;
		this.border.width = f;
	...
	mb.prototype._render = function(b) {
		...
		for (var d = [], c = 0; c < this.items.length; c++) {
			var f = this.items[c], e = f._render(b);
			a.merge(d, e)
		}
		return d
	};
	...
	hb.prototype._arrange = function() {
		var a = this.marker.size / 2, b = this.x + a, c = this.y + a
				+ (this.height - this.marker.size) / 2;
	...
	B.prototype._arrangeRenderer = function(a) {
		if (!e.use_excanvas)
			return;
		var f = this.x, g = this.y, b = a.canvas, h = a.div, d = Math.max(
				this.width, 0), c = Math.max(this.height, 0);
		h.css({
			left : f,
			top : g,
			width : d,
	...
	B.prototype._getStripes = function(b) {
		...
		a.each(d, function() {
			...
			if (a.width)
				...
				j = b.actualInterval;
			for (var h = a.lineWidth, o = h / 2, n = b
					._getIntervals(k, a, true), m = 0; m < n.length; m++) {
				var l = n[m];
				if (l >= b.actualVisibleMaximum)
					continue;
				var d = b.getPosition(l), p = b._getNextPosition(l, j), f = b
						.getPosition(p), g;
				if (b.isAxisVertical)
					g = new i(c.x + o, Math.min(d, f), c.width - h, Math.abs(f
							- d));
				else
					g = new i(Math.min(d, f), c.y + o, Math.abs(f - d),
							c.height - h);
				a._setSettings(g);
				e.push(g)
			}
	...
	B.prototype._getGridLines = function(a, b, j) {
		if (a.location == "radial" || b == null || b.visible != true)
			return [];
		for (var h = [], m = a._getMarkInterval(b, j), i = a._getIntervals(m,
				b, j), c, d, e, f, g = 0; g < i.length; g++) {
	...
	B.prototype._getPlotLines = function(h) {
		...
		for (var e = this, j = [], k = [], i = 0; i < g.length; i++) {
			...
			d.setOptions(g[i]);
			var f = d.lineWidth / 2, m = h.getPosition(d.value), c = new X(
					d.title);
			c.chart = this.chart;
			c._measure();
			if (h.isAxisVertical) {
	...
	B.prototype._getPlotBands = function(g) {
		...
		for (var l = this, q = [], r = [], p = 0; p < k.length; p++) {
			...
			if (!g.isAxisVertical)
				d.defaults.title.hAlign = "right";
			d.setOptions(k[p]);
			var o = d.lineWidth, t = o / 2, m = g.getPosition(d.from), n = g
					.getPosition(d.to), h, j, f, e;
			if (g.isAxisVertical) {
				h = l.x + t;
				j = Math.min(m, n);
				f = l.width - o;
	...
	B.prototype._contains = function(a, b) {
		return a >= this.x && a <= this.x + this.width && b >= this.y
	...
	w.prototype._findStackedClusters = function(h, e) {
		for (var f = h.stackedGroupName, g = this._getStackedGroupsFromType(e), l = a
				.inArray(f, g), k = g.length, i = -1, b = 0, j = this.items, d = 0; d < j.length; d++) {
			var c = j[d];
			if (c == h)
				i = b;
	...
	w.prototype._getPixelMargins = function(f) {
		for (var b = 0, a = 0, e = this.items, c = 0; c < e.length; c++) {
			var g = e[c], d = g._getPixelMargins(f);
			b = Math.max(b, d.left);
	...
	f.prototype = {
		...
		_addMarker : function(d, e, i, c, h) {
			var a = this._correctMarkerPosition(d, e, c), f = this._getMarker(
					a.x, a.y, i, c, h), g = this.markers.offset, b = null;
			if (g) {
				b = new n(d, e, a.x, a.y);
		...
		_addMarkerAndLabel : function(f, n, j, k, a, h, i, c, m, d) {
			...
			if (this.markers && this.markers.isVisible()
					&& this.realYAxis.isValueVisible(c)) {
				if (!d)
					d = {
						chart : this.chart,
						series : this,
				...
				var o = d.dataItem[2], b = this._addMarker(j, k, null, c, o);
				if (b.marker) {
					b.marker.context = d;
			...
			if (this.labels && this.labels.visible !== false) {
				var l = this._getLabelValue(c, a), e = this._getDataPointLabel(
						c, j, k, m + g, l);
				e.context = {
					chart : this.chart,
					series : this,
		...
		_addShapeAnimation : function(e, g, f) {
			...
			if (!b || b.enabled === false)
				return;
			var a = new ob(b, e, "visible", false, true), c = a.duration / f, d = a.delayTime
					+ g * c;
			a.delayTime = d;
			a.duration = c;
		...
		_setMarkerSettings : function(a) {
			...
			if (b.isNull(a.shadowColor))
				a.shadowColor = !b.isNull(this.shadowColor) ? this.shadowColor
						: c.shadowColor;
			if (b.isNull(a.shadowBlur))
				a.shadowBlur = !b.isNull(this.shadowBlur) ? this.shadowBlur
						: c.shadowBlur;
			if (b.isNull(a.shadowOffsetX))
				a.shadowOffsetX = !b.isNull(this.shadowOffsetX) ? this.shadowOffsetX
						: c.shadowOffsetX;
			if (b.isNull(a.shadowOffsetY))
				a.shadowOffsetY = !b.isNull(this.shadowOffsetY) ? this.shadowOffsetY
						: c.shadowOffsetY
		},
		_setMarkerLinkLineSettings : function(a) {
			a.strokeStyle = a.strokeStyle || this.markers.strokeStyle
					|| this.fillStyle
		},
		...
		_initVisibleXYData : function() {
			if (this.realYAxis.zoomEnabled)
				return;
			var a = this.realXAxis, e = a.visibleMinimum || a.minimum, d = a.visibleMaximum
					|| a.maximum, m = !b.isNull(e), l = !b.isNull(d);
			if (!m && !l)
				return;
			if (a.skipEmptyDays) {
		...
		_initVisibleCatValueData : function() {
			if (this.realYAxis.zoomEnabled)
				return;
			var e = this.realXAxis, o = e.visibleMinimum || e.minimum, n = e.visibleMaximum
					|| e.maximum, l = !b.isNull(o), j = !b.isNull(n);
			if (!l && !j)
				return;
		...
		_initVisibleCatValueDataRange : function(p, r) {
			if (this.realYAxis.zoomEnabled)
				return;
			var c = this.realXAxis, n = c.visibleMinimum || c.minimum, m = c.visibleMaximum
					|| c.maximum, j = !b.isNull(n), i = !b.isNull(m);
			if (!j && !i)
				return;
		...
		_initVisibleXYDataRange : function(q, s) {
			if (this.realYAxis.zoomEnabled)
				return;
			var a = this.realXAxis, d = a.visibleMinimum || a.minimum, c = a.visibleMaximum
					|| a.maximum, n = !b.isNull(d), m = !b.isNull(c);
			if (!n && !m)
				return;
			if (a.skipEmptyDays) {
				d = a._addEmptyDaysOffset(d);
		...
		_initStackedData : function(b) {
			...
			if (a.isArray(c) == false)
				return;
			var g = this.chart.series._findStackedClusters(this, b), e = this.chart.series
					._getStackedSeriesFromType(b, this.stackedGroupName), d = this
					._calcStackedData(c, g, e);
			a.extend(this, d)
		},
		...
		_calcVisibleStackedData : function(s, w, u) {
			if (this.realYAxis.zoomEnabled)
				return;
			var e = this.realXAxis, j = e.visibleMinimum || e.minimum, i = e.visibleMaximum
					|| e.maximum, m = !b.isNull(j), l = !b.isNull(i);
			if (!m && !l)
				return;
			if (e.skipEmptyDays) {
		...
		_getDataPointLabel : function(i, j, d, c, f) {
			var g = i >= this.realYAxis.getCrossing(), h = this
					._getLabelText(f), b = new q(h);
		...
		_getTooltip : function(b) {
			...
			if (this.title) {
				...
				a = '<span style="color:' + d + '">' + this.title + "</span>: "
						+ a
			}
			return a
		},
		_calcColumnScale : function(k) {
			for (var e = [], b = 0; b < k.length; b++)
				a.merge(e, k[b].data || []);
			e.sort(function(a, b) {
				return a[0] - b[0]
			});
			...
			var i = this.realXAxis, j = i.series, f = j.maxX - j.minX;
			if (i.skipEmptyDays)
				f -= i.totalEmptyDaysTicks;
			f = Math.min(.5, g / f);
			return f
		},
		...
		getPercentage : function(a, d) {
			var b = this._getTotal(this.type, d), c = a > 0 ? b.positive
					: b.negative;
	...
	L.prototype._renderCatValueData = function(j) {
		for (var o = this.data.length, f = this.markers != null
				&& this.markers.isVisible(), c = [], i = [], n = [], l = [], q = f ? this.markers.size / 2
				: 0, h, k, p, d, e = 0; e < o; e++) {
			var g = this.data[e];
			if (g === null) {
				c.push(null);
	...
	L.prototype._renderXYData = function(i) {
		var k = this.data.length, d = this.markers != null
				&& this.markers.isVisible(), q = this.labels != null
				&& this.labels.visible !== false;
		if (k > 1e3 && d == false && q == false) {
			this._renderLargeXYData(i);
			return
		}
		for (var c = [], h = [], p = [], n = [], r = d ? this.markers.size / 2
				: 0, l, m, f, e, g = 0; g < k; g++) {
			var j = this.data[g];
			if (j === null) {
				c.push(null);
	...
	L.prototype._renderLargeXYData = function(s) {
		for (var x = this.data.length, w = this.chart.gridArea.width, r = this.chart.gridArea.height, i = this.realXAxis, j = this.realYAxis, t = 2
				* (i.actualVisibleMaximum - i.actualVisibleMinimum) / w, u = 2
				* (j.actualVisibleMaximum - j.actualVisibleMinimum) / r, a = [], p, q, c, b, n = 0, o = 0, k = 0, l = 0, d, f, g, v = 0, e = [], h = 0; h < x; h++) {
			d = this.data[h];
			if (d === null) {
				a.push(null);
				a.push(null);
	...
	ab.prototype._createShape = function(i, h) {
		...
		var f = this.chart.gridArea, k = f.y, j = f.y + f.height, e = this.realYAxis
				.getCrossingPosition();
	...
	p.prototype._renderCatData = function(p) {
		var j = this.chart.gridArea, y = j.x, D = y + j.width, e = this.realYAxis
				.getCrossingPosition();
		e = b.fitInRange(e, y, D);
		e = Math.round(e);
		var q = this.chart.series._findClusters(this, this.type), m = this.data.length, B = j.height
				/ this.realXAxis.getZoom(), w = B / m, E = w / q.count, h = Math
				.round(this.pointWidth * E), A = q.count * h, o = (w - A) / 2;
		o = Math.round(o + q.index * h);
		for (var u = [], r = [], C = this.markers && this.markers.isVisible() ? this.markers.size / 2 + 2
				: 2, c, l, d = 0; d < m; d++) {
			var n = this.data[d];
			if (n == null)
				continue;
	...
	p.prototype._renderLinearData = function(l) {
		var g = this.chart.gridArea, w = g.x, D = w + g.width, d = this.realYAxis
				.getCrossingPosition();
		d = b.fitInRange(d, w, D);
		d = Math.round(d);
		for (var m = this.data.length, z = g.height / this.realXAxis.getZoom(), C = this
				._getSeriesFromThisType(), B = this._calcColumnScale(C) * z, o = this.pointWidth
				* B, t = [], q = [], A = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 : 0, c, p, e = 0; e < m; e++) {
			var k = this.data[e];
	...
	p.prototype._addAnimation = function(d, g, f) {
		...
		if (!b || b.enabled === false)
			return;
		var a = new jb(b, d, "xDecrease", d.width, 0), c = a.duration / f, e = a.delayTime
				+ g * c;
		a.delayTime = e;
		a.duration = c;
		this.chart.storyboard.addAnimation(a)
	...
	p.prototype._getPixelMargins = function(a) {
		if (a.isVertical() == false) {
			var b = f.prototype._getPixelMargins.call(this, a), c = a.length / 10, k = Math
					.max(c, b.left), j = Math.max(c, b.right);
			return {
				left : k,
				right : j
			}
		...
		var h = 4, i = this._getSeriesFromThisType(), e = this
				._calcColumnScale(i), g = a.length, d = .5 * e * g + h;
		return {
			left : d,
			right : d
		}
	...
	p.prototype._getDataPointLabel = function(k, g, l, f, h) {
		var i = k <= this.realYAxis.getCrossing(), j = this._getLabelText(h), b = new q(
				j);
		e.setShadows(b, this, this.chart);
		a.extend(b, this.labels);
		b.measure(this.chart.ctx);
	...
	Z.prototype._render = function(l) {
		if (!this.data)
			return;
		var n = this.chart, o = n.options, g = o.maxBubbleSize;
		if (!g) {
			var q = n.gridArea;
		...
		for (var i = this.data.length, j, k, e, c, b, a = 0; a < i; a++) {
			...
			if (this.labels && this.labels.visible !== false) {
				var r = this._getLabelValue(b, a), m = this._getDataPointLabel(
						b, j, k, p / 2, r);
				l.push(m);
	...
	Z.prototype._getTooltip = function(b) {
		var a = "y: <b>" + b.y.toString() + "</b></br>size: <b>"
				+ b.size.toString() + "</b>";
		if (this.title) {
			var d = c.getColorFromFillStyle(this.fillStyle);
	...
	G.prototype._renderCatData = function(p) {
		var k = this.chart.gridArea, B = k.y, A = k.y + k.height, d = this.realYAxis
				.getCrossingPosition();
		d = b.fitInRange(d, B, A);
		d = Math.round(d);
		var q = this.chart.series._findClusters(this, this.type), l = this.data.length, x = k.width
				/ this.realXAxis.getZoom(), u = x / l, z = u / q.count, j = Math
				.round(this.pointWidth * z), w = q.count * j, o = (u - w) / 2;
		o = Math.round(o + q.index * j);
		for (var t = [], r = [], y = this.markers && this.markers.isVisible() ? this.markers.size / 2
				: 0, f, g, c = 0; c < l; c++) {
			var m = this.data[c];
			if (m == null)
				continue;
	...
	G.prototype._renderLinearData = function(m) {
		var j = this.chart.gridArea, z = j.y, x = j.y + j.height, c = this.realYAxis
				.getCrossingPosition();
		c = b.fitInRange(c, z, x);
		c = Math.round(c);
		for (var n = this.data.length, u = j.width / this.realXAxis.getZoom(), y = this
				._getSeriesFromThisType(), w = this._calcColumnScale(y) * u, o = this.pointWidth
				* w, r = [], p = [], v = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 : 0, e, f, d = 0; d < n; d++) {
			var k = this.data[d];
			if (k == null || a.isArray(k) == false)
				continue;
			e = k[0];
	...
	G.prototype._addAnimation = function(d, g, f) {
		...
		if (!b || b.enabled === false)
			return;
		var a = new jb(b, d, "yDecrease", d.height, 0), c = a.duration / f, e = a.delayTime
				+ g * c;
		a.delayTime = e;
		a.duration = c;
		this.chart.storyboard.addAnimation(a)
	...
	A.prototype._render = function(v) {
		if (!this.data)
			return;
		var k = this.chart.gridArea, r = k.width, p = k.height, j = this
				._getYValues(), o = b.sum(j), l = j.length, u = 10, h;
		if (r < p)
			h = r / 2 - u;
		else
			h = p / 2 - u;
		...
		for (var y = k.x + r / 2, z = k.y + p / 2, x = Math.PI * 2 / o, f = -Math.PI / 2, m = this.fillStyles, c = 0; c < l; c++) {
			var e = j[c], n = 100 * e / o, i = f + e * x, g = new qb(y, z, h,
					f, i);
		...
		for (var f = -Math.PI / 2, c = 0; c < l; c++) {
			var e = j[c], n = 100 * e / o, s = e, i = f + e * x;
			switch (this.labels.valueType) {
			case "percentage":
				s = n
			}
			if (e == 0)
				continue;
			var B = this._getLabelText(s), d = new q(B);
			d.textBaseline = "top";
			a.extend(d, this.labels);
			var w = d.measure(this.chart.ctx), t = this._getSliceCenter(y, z,
					(f + i) / 2, h * .6);
			d.x = t.x - w.width / 2;
			d.y = t.y - w.height / 2;
			d.context = {
	...
	A.prototype._getTooltip = function(a) {
		var d = this.chart.stringFormat(a.percentage, "%.2f%%"), b = "<b>"
				+ a.value + " (" + d + ")</b><br/>", c = a.dataItem[0];
		if (c)
			b = c + "</br>" + b;
		return b
	};
	A.prototype._addSliceAnimation = function(c, g, f) {
		...
		if (!b || b.enabled === false)
			return;
		var a = new jb(b, c, "endAngle", c.startAngle, c.endAngle), d = a.duration
				/ f, e = a.delayTime + g * d;
		a.delayTime = e;
		a.duration = d;
		this.chart.storyboard.addAnimation(a)
	...
	A.prototype.getTotal = function() {
		var a = this._getYValues(), c = b.sum(a);
		return c
	};
	...
	x.prototype._renderCatData = function(w) {
		if (!this.data)
			return;
		var r = this.chart.gridArea, N = r.y, M = r.y + r.height, h = this.realYAxis
				.getCrossingPosition();
		h = b.fitInRange(h, N, M);
		h = Math.round(h);
		var c = this.chart.series._findStackedClusters(this, this.type), F = this.data.length, I = r.width
				/ this.realXAxis.getZoom(), D = I / F, L = D / c.groupCount, m = Math
				.round(this.pointWidth * L), H = c.groupCount * m, v = (D - H) / 2;
		v = Math.round(v + c.groupIndex * m);
		for (var C = [], B = [], s = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), J = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 : 0, f, g, d, e = 0; e < F; e++) {
			d = this.data[e];
			if (d == null)
				continue;
			...
			var o = this._getPrevStackedPosition(s, c.index, q, h,
					this.realYAxis, l >= 0), x = g, p;
			if (g <= o)
				p = o - g;
			else {
				p = g - o;
			...
			var n = d.actualValue, E = this._getStackedTotal(s, q), G = n > 0 ? E.positive
					: E.negative, z = G != 0 ? 100 * Math.abs(n) / Math.abs(G)
					: 0, t = {
				chart : this.chart,
				series : this,
				dataItem : this.data[e],
			...
			if (this.labels && this.labels.visible !== false
				...
				var A = this.labels.position == "outside", K = A ? J : -p / 2, j = this
						._getDataPointLabel(d.actualValue, f, x, K, y);
				if (!A)
	...
	x.prototype._renderLinearData = function(u) {
		if (!this.data)
			return;
		var r = this.chart.gridArea, L = r.y, I = r.y + r.height, h = this.realYAxis
				.getCrossingPosition();
		h = b.fitInRange(h, L, I);
		h = Math.round(h);
		for (var e = this.chart.series._findStackedClusters(this, this.type), m = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), K = this.data.length, E = r.width
				/ this.realXAxis.getZoom(), H = this._calcColumnScale(m) * E, w = this.pointWidth
				* H, B = [], A = [], F = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 : 0, f, g, c, d = 0; d < K; d++) {
			c = this.data[d];
			if (c == null)
				continue;
			...
			g = Math.round(this.realYAxis.getPosition(l));
			var o = this._getPrevStackedPosition(m, e.index, q, h,
					this.realYAxis, l >= 0), v = g, p;
			if (g <= o)
				p = o - g;
			else {
				p = g - o;
			...
			var n = c.actualValue, C = this._getStackedTotal(m, q), D = n > 0 ? C.positive
					: C.negative, y = D != 0 ? 100 * Math.abs(n) / Math.abs(D)
					: 0, s = {
				chart : this.chart,
				series : this,
				dataItem : this.data[d],
			...
			if (this.labels && this.labels.visible !== false
				...
				var z = this.labels.position == "outside", G = z ? F : -p / 2, j = this
						._getDataPointLabel(c.actualValue, f, v, G, x);
				if (!z)
					j.textBaseline = "middle";
				j.context = s;
				this.chart.elem.trigger("dataPointLabelCreating", j);
	...
	x.prototype._getSeriesFromThisType = function() {
		return this.chart.series._getStackedSeriesFromType(this.type,
				this.stackedGroupName)
	};
	...
	y.prototype._renderCatData = function(v) {
		if (!this.data)
			return;
		var w = this.chart.gridArea, F = w.x, M = F + w.width, k = this.realYAxis
				.getCrossingPosition();
		k = b.fitInRange(k, F, M);
		k = Math.round(k);
		var c = this.chart.series._findStackedClusters(this, this.type), G = this.data.length, J = w.height
				/ this.realXAxis.getZoom(), D = J / G, N = D / c.groupCount, h = Math
				.round(this.pointWidth * N), I = c.groupCount * h, u = (D - I) / 2;
		u = Math.round(u + c.groupIndex * h);
		for (var C = [], B = [], r = this.chart.series
				._getStackedSeriesFromType(this.type, this.stackedGroupName), K = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 + 2 : 2, f, j, d, e = 0; e < G; e++) {
			d = this.data[e];
			if (d == null)
				continue;
			...
			var o = this._getPrevStackedPosition(r, c.index, p, k,
					this.realYAxis, m >= 0), x = f, q;
			if (f <= o)
				q = o - f;
			else {
				q = f - o;
			...
			var n = d.actualValue, E = this._getStackedTotal(r, p), H = n > 0 ? E.positive
					: E.negative, z = H != 0 ? 100 * Math.abs(n) / Math.abs(H)
					: 0, s = {
				chart : this.chart,
				series : this,
				dataItem : this.data[e],
			...
			if (this.labels && this.labels.visible !== false
				...
				var A = this.labels.position == "outside", L = A ? K : -q / 2, g = this
						._getDataPointLabel(d.actualValue, x, j, L, y);
				if (!A) {
	...
	y.prototype._renderLinearData = function(t) {
		if (!this.data)
			return;
		var u = this.chart.gridArea, D = u.x, K = D + u.width, h = this.realYAxis
				.getCrossingPosition();
		h = b.fitInRange(h, D, K);
		h = Math.round(h);
		for (var q = this.chart.series._getStackedSeriesFromType(this.type,
				this.stackedGroupName), f = this.chart.series
				._findStackedClusters(this, this.type), M = this.data.length, F = u.height
				/ this.realXAxis.getZoom(), J = this._getSeriesFromThisType(), I = this
				._calcColumnScale(J)
				* F, w = this.pointWidth * I, B = [], A = [], G = this.markers
				&& this.markers.isVisible() ? this.markers.size / 2 + 2 : 2, e, l, c, d = 0; d < M; d++) {
			c = this.data[d];
			if (c == null)
				continue;
			...
			var n = this._getPrevStackedPosition(q, f.index, o, h,
					this.realYAxis, k >= 0), v = e, p;
			if (e <= n)
				p = n - e;
			else {
				p = e - n;
			...
			var m = c.actualValue, C = this._getStackedTotal(q, o), E = m > 0 ? C.positive
					: C.negative, y = E != 0 ? 100 * Math.abs(m) / Math.abs(E)
					: 0, r = {
				chart : this.chart,
				series : this,
				dataItem : this.data[d],
			...
			if (this.labels && this.labels.visible !== false
				...
				var z = this.labels.position == "outside", H = z ? G : -p / 2, g = this
						._getDataPointLabel(c.actualValue, v, l, H, x);
				if (!z) {
					g.textBaseline = "middle";
					g.textAlign = "center"
				}
	...
	db.prototype._getPixelMargins = function(b) {
		if (b.isVertical() || this.data == null)
			return {
				left : 0,
				right : 0
			};
		var e = 4, f = this._getSeriesFromThisType(), c = this
				._calcColumnScale(f), d = b.length, a = .5 * c * d + e;
		return {
			left : a,
			right : a
		}
	...
	db.prototype._getTooltip = function(b) {
		var d = this.chart.stringFormat(b.percentage, "%.2f%%"), a = "<b>"
				+ b.value + " (" + d + ")</b><br/>";
		if (this.title) {
	...
	eb.prototype._getPixelMargins = function(b) {
		if (b.isVertical() == false || this.data == null)
			return {
				left : 0,
				right : 0
			};
	...
	J.prototype._renderCatData = function(u) {
		var v = this.chart.gridArea, b = this.chart.series._findClusters(this,
				this.type), g = this.data.length;
		b.count = 1;
		b.index = 0;
		var q = v.width / this.realXAxis.getZoom(), m = q / g, r = m / b.count, l = Math
				.round(this.pointWidth * r), p = b.count * l, d = (m - p) / 2;
		...
		for (var o, j, k, n, e, f, a = 0; a < g; a++) {
			...
			var s = {
				...
				from : e,
				to : f
			}, w = Math.min(j, k), t = Math.abs(j - k), c = new i(o, w, l, t);
			c.context = s;
	...
	J.prototype._renderLinearData = function(r) {
		for (var s = this.chart.gridArea, u = this.data.length, m = s.width
				/ this.realXAxis.getZoom(), q = this._getSeriesFromThisType(), n = this
				._calcColumnScale(q)
				* m, k = this.pointWidth * n, l, h, j, g, e, f, b = 0; b < u; b++) {
			var c = this.data[b];
			...
			var t = l - k / 2, v = Math.min(h, j), p = Math.abs(h - j), d = new i(
					t, v, k, p);
			d.context = o;
	...
	J.prototype._getTooltip = function(b) {
		var a = "From: <b>" + b.from.toString() + "</b></br>To: <b>"
				+ b.to.toString() + "</b>";
		if (this.title) {
			var d = c.getColorFromFillStyle(this.fillStyle);
			a = '<div style="color:' + d + '">' + this.title + "</div>" + a
	...
	r.prototype._renderCatData = function(s) {
		var t = this.chart.gridArea, b = this.chart.series._findClusters(this,
				this.type), h = this.data.length;
		b.count = 1;
		b.index = 0;
		var q = t.height / this.realXAxis.getZoom(), m = q / h, v = m / b.count, d = Math
				.round(this.pointWidth * v), p = b.count * d, e = (m - p) / 2;
		...
		for (var k, l, o, n, f, g, a = 0; a < h; a++) {
			...
			var r = {
				...
				from : f,
				to : g
			}, w = Math.min(k, l), u = Math.abs(k - l), c = new i(w, o, u, d);
			c.context = r;
			this._setShapeSettings(c, a);
			s.push(c);
	...
	r.prototype._renderLinearData = function(r) {
		for (var s = this.chart.gridArea, k = this.data.length, n = s.height
				/ this.realXAxis.getZoom(), q = this._getSeriesFromThisType(), p = this
				._calcColumnScale(q)
				* n, l = this.pointWidth * p, h, j, m, g, e, f, b = 0; b < k; b++) {
			var c = this.data[b];
			if (c == null || a.isArray(c) == false)
				continue;
			g = c[0];
			...
			h = Math.round(this.realYAxis.getPosition(e));
			j = Math.round(this.realYAxis.getPosition(f));
			var u = m - l / 2, v = Math.min(h, j), t = Math.abs(h - j), d = new i(
					v, u, t, l);
			d.context = o;
			this._setShapeSettings(d, b);
			r.push(d);
	...
	r.prototype._getTooltip = function(b) {
		var d = this.realYAxis, a = "From: " + d._getTooltip(b.from) + "To: "
				+ d._getTooltip(b.to);
		if (this.title) {
			var e = c.getColorFromFillStyle(this.fillStyle);
	...
	H.prototype._render = function(p) {
		if (!this.data)
			return;
		var q = this.chart.gridArea, j = this.data.length, d, f, h, g, e, l = q.width
				/ this.realXAxis.getZoom(), i;
		if (this.xAxisType == "CategoryAxis")
			i = this.pointWidth * l / j;
		else {
	...
	H.prototype._getPixelMargins = function(a) {
		...
		var e = 4, c = this._calcColumnScale([ this ]), d = a.length, b = .5
				* c * d + e;
		return {
			left : b,
			right : b
	...
	H.prototype._getTooltip = function(a) {
		var b = "Open: <b>" + a.open.toString() + "</b></br>High: <b>"
				+ a.high.toString() + "</b></br>Low: <b>" + a.low.toString()
				+ "</b></br>Close: <b>" + a.close.toString() + "</b>";
		if (this.title) {
			var d = c.getColorFromFillStyle(this.fillStyle);
	...
	E.prototype._render = function(l) {
		if (!this.data)
			return;
		for (var x = this.data.length, i = this.markers != null
				&& this.markers.isVisible(), u = this.labels != null
				&& this.labels.visible !== false, e = [], j = [], p = [], n = [], v = i ? this.markers.size / 2
				: 0, g, h, d, y = this.realYAxis.cx, r = this.realYAxis.cy, c = 0; c < x; c++) {
			var f = this.data[c];
			if (f === null) {
				e.push(null);
				e.push(null);
			...
			if (u) {
				var t = this._getLabelValue(d, c), m = this._getDataPointLabel(
						d, g, h, v, t);
				m.context = o;
	...
	z.prototype._render = function(m) {
		if (!this.data)
			return;
		for (var y = this.data.length, h = this.markers != null
				&& this.markers.isVisible(), v = this.labels != null
				&& this.labels.visible !== false, c = [], j = [], q = [], o = [], w = h ? this.markers.size / 2
				: 0, f, g, i, d, z = this.realYAxis.cx, s = this.realYAxis.cy, e = 0; e < y; e++) {
			var k = this.data[e];
			if (k === null) {
				c.push(null);
				c.push(null);
			...
			if (v) {
				var u = this._getLabelValue(d, e), n = this._getDataPointLabel(
						d, f, g, w, u);
				n.context = p;
	...
	R.prototype._getRegression = function(m, g) {
		var h = this.trendlineType, e = this.data.length, b = 0, f = 0, i = 0, j = 0, l = 0, d = [], c = [];
		if (h == "linear") {
			c = m;
			d = g
		...
		var k = (e * j - b * f) / (e * i - b * b), n = (f - k * b) / e;
		return [ k, n ]
	};
	...
	R.prototype._getExpRegression = function(d, e) {
		var a = this._getRegression(d, e), c = Math.exp(a[0]), b = Math
				.exp(a[1]);
		return [ c, b ]
	};
	...
	K.prototype._renderXYData = function(j) {
		for (var p = this.data.length, e = this.markers != null
				&& this.markers.isVisible(), r = this.labels != null
				&& this.labels.visible !== false, c = [], h = [], o = [], m = [], q = e ? this.markers.size / 2
				: 0, k, l, f, d, g = 0; g < p; g++) {
			var i = this.data[g];
			if (i === null) {
				c.push(null);
	...
	Y.prototype._createShape = function(i, h) {
		...
		var c = new C(g);
		this._setShapeSettings(c);
		var f = this.chart.gridArea, k = f.x, j = f.x + f.width, e = this.realYAxis
				.getCrossingPosition();
		e = b.fitInRange(e, k, j);
	...
	t.prototype._renderCatValueData = function(k) {
		for (var l = this.data.length, e = this.markers != null
				&& this.markers.isVisible(), c = [], d = [], f = [], r = [], p = [], s = e ? this.markers.size / 2
				: 0, j, n, o, m, g, h, b = 0; b < l; b++) {
			var i = this.data[b];
			if (i === null) {
				c.push(null);
				c.push(null);
	...
	t.prototype._renderXYData = function(i) {
		for (var l = this.data.length, d = this.markers != null
				&& this.markers.isVisible(), t = this.labels != null
				&& this.labels.visible !== false, j = [], k = [], e = [], r = [], p = [], s = d ? this.markers.size / 2
				: 0, h, n, o, m, f, g, b = 0; b < l; b++) {
			var c = this.data[b];
			if (c === null) {
				pts.push(null);
				pts.push(null);
	...
	t.prototype._addMarkersAndLabels = function(g, m, i, p, q, b, h, o, d, e, l) {
		...
		if (this.markers && this.markers.isVisible()) {
			...
			var n = f.dataItem[3], a = this._addMarker(i, p, null, d, n);
			if (a.marker) {
				a.marker.context = f;
		...
		if (this.labels && this.labels.visible !== false) {
			var j = this._getLabelValue(d, b), c = this._getDataPointLabel(d,
					i, p, l + k, j, d > e), f = {
				chart : this.chart,
				series : this,
				dataItem : this.data[b],
	...
	t.prototype._getDataPointLabel = function(j, i, d, c, f, g) {
		var h = this._getLabelText(f), b = new q(h);
		e.setShadows(b, this, this.chart);
		a.extend(b, this.labels);
		b.textAlign = "center";
		b.x = i;
	...
	l.prototype._arrange = function() {
		...
		var e = this.offset;
		if (!this.jqRangeSlider) {
			var d = a('<div style="position:absolute"></div>')
					.jqRangeSlider({}), f = this;
	...
	l.prototype._moveVisibleRange = function(f, g) {
		var e = this.isAxisVertical, c = this.actualVisibleMinimum, b = this.actualVisibleMaximum, d = b
				- c, a = 0;
		if (e)
			a = -d * g / this.length;
		else
	...
	l.prototype._mouseWheelZoom = function(f) {
		var b = this.actualVisibleMinimum, a = this.actualVisibleMaximum, c = (this.actualMaximum - this.actualMinimum) / 1e3, e = f
				* 50 * c;
	...
	l.prototype._scaleVisibleRange = function(a, f) {
		var e = this.actualVisibleMinimum, d = this.actualVisibleMaximum, h = d
				- e, i = this.getZoom(), l, b, c, m = this.isAxisVertical;
		if (m) {
			l = a.dy / f.dy;
			var p = f.y1 - a.y1, q = f.y2 - a.y2;
			b = -h * q / this.length / i;
			c = -h * p / this.length / i;
			if (a.y1 > a.y2) {
				var j = b;
		...
		var k = (e + d) / 2, g = (this.actualMaximum - this.actualMinimum) / 1e3;
	...
	l.prototype._scaleToRegion = function(j, k) {
		var c = this.chart.gridArea, d = c.fitHor(j.locX), f = c.fitVer(j.locY), e = c
				.fitHor(k.locX), g = c.fitVer(k.locY), i = this.reversed, a, b;
		if (this.isAxisVertical)
			if (i) {
				a = Math.min(f, g);
				b = Math.max(f, g)
	...
	F.prototype._getLabelIntervals = function(d, c) {
		...
		if (c && c.intervalOffset)
			e = c.intervalOffset;
		for (var f = [], h = Math.round(this.actualVisibleMinimum), g = this
				._getIntervalStart(h, d) + .5, a = g + e; a <= this.actualVisibleMaximum; a = b
				.round(a + d))
			f.push(a);
		return f
	};
	...
	m.prototype._initRange = function() {
		...
		if (this.skipEmptyDays)
			c -= this.totalEmptyDaysTicks;
		var n = Math.abs(c - g);
		if (n == 0)
			n = 1;
		var m = 0, l = 0, e = i._getPixelMargins(this);
		if (this.isAxisVertical) {
			e.left = b.isNull(this.bottomMargin) ? e.left + .5
					: this.bottomMargin;
			e.right = b.isNull(this.topMargin) ? e.right + .5 : this.topMargin
		} else {
			e.left = b.isNull(this.leftMargin) ? e.left + .5 : this.leftMargin;
			e.right = b.isNull(this.rightMargin) ? e.right + .5
					: this.rightMargin
		}
		var o = n / this.length;
		m = o * e.left;
		...
		var a = g - m, d = c + l, r = this.series._isAnchoredToOrigin(), f = this
				.getCrossing();
		...
		this._setVisibleRanges();
		if (this.logarithmic === true)
			this.actualInterval = this._calculateActualIntervalLogarithmic(
					this.actualVisibleMinimum, this.actualVisibleMaximum);
		else
			this.actualInterval = this._calculateActualInterval(
					this.actualVisibleMinimum, this.actualVisibleMaximum)
	};
	...
	m.prototype._calculateActualIntervalLogarithmic = function(e, d) {
		if (this.interval)
			return this.interval;
		var c = (d - e) / 3, a = Math.floor(b.log10(Math.abs(c)));
		if (a == 0)
			a = 1;
	...
	m.prototype._getIntervals = function(c, a, h) {
		if (this.logarithmic === false)
			return l.prototype._getIntervals.call(this, c, a);
		if (h === false)
			return this._getLogarithmicMinorIntervals(c, a);
		var e = 0;
		if (a && a.intervalOffset)
			e = a.intervalOffset;
		for (var f = [], g = this._getIntervalStart(this.actualVisibleMinimum,
				c), d = g + e; d <= this.actualVisibleMaximum; d = b.round(d
				+ c))
			f.push(Math.pow(this.logBase, d));
		return f
	};
	m.prototype._getLogarithmicMinorIntervals = function(m, k) {
		for (var l = this._getMarkInterval(k.major, true), h = this
				._getIntervals(l, k.major, true), j = [], d = null, g = 0; g < h.length; g++) {
			var f = h[g];
			if (d == null) {
				d = f;
				continue
	...
	function j(c) {
		var b = a.jqChartDateFormat.masks, d = a.extend(true, {},
				this.defaults, {
					labels : {
						...
						hoursIntervalStringFormat : b.shortDate + " "
								+ b.shortTime,
	...
	j.prototype._calculateDateTimeInterval = function(j, i) {
		var h = i - j, f = .8 * this.maxInter200Px, g = Math
				.max(1, this.length), e = g / (200 * 10 / f), b = h / e;
		this.type = "year";
		var a = b / (1e3 * 60);
		if (a <= 1) {
			if (b <= 10) {
	...
	j.prototype._getIntervalStart = function(j, b, e) {
		...
		if (b > 0 && b != 1)
			if (e == "months" && b <= 12 && b > 1) {
				var i = a, c = new Date(a.getFullYear(), 0, 1, 0, 0, 0);
				while (c < a) {
					i = c;
					c = d.addMonths(c, b)
				}
		...
		case "minutes":
			...
			a = new Date(a.getFullYear(), a.getMonth(), a.getDate(), a
					.getHours(), l, 0);
			break;
		case "seconds":
			var m = a.getSeconds() / b * b;
			a = new Date(a.getFullYear(), a.getMonth(), a.getDate(), a
					.getHours(), a.getMinutes(), m, 0);
			break;
		case "milliseconds":
			var k = a.getMilliseconds() / b * b;
			a = new Date(a.getFullYear(), a.getMonth(), a.getDate(), a
					.getHours(), a.getMinutes(), a.getSeconds(), k);
			break;
		case "weeks":
	...
	j.prototype._getIntervals = function(f, c) {
		var i = [], e = this.actualVisibleMinimum, g = this.actualVisibleMaximum;
		if (this.skipEmptyDays) {
		...
		if (c && c.intervalOffset) {
			var h = this.actualIntervalType, j = c.intervalOffset;
			if (c.intervalOffsetType)
				h = c.intervalOffsetType;
			a = this._incrementDateTime(a, j, h)
		}
		for (var b = a; b <= g; b = this._incrementDateTime(b, f,
	...
	j.prototype._getNextNonEmptyDay = function(b) {
		...
		if (e == -1)
			return b;
		var h = d.addDays(new Date(b), 1), g = this.actualMaximum;
		if (this.skipEmptyDays)
	...
	j.prototype._addEmptyDaysOffset = function(c) {
		if (b.isNull(c))
			return null;
		var a = c + this._getEmptyDaysOffset(c), d = this
				._getEmptyDaysOffset(a)
				- this._getEmptyDaysOffset(c);
		c = a;
		a += d;
		while (d) {
	...
	j.prototype._excludeDaysFromSeries = function(c, b) {
		a.each(c.data, function(g, e) {
			var f = d.roundToDay(e[0]).getTime(), c = a.inArray(f, b);
			c != -1 && b.splice(c, 1)
		})
	};
	j.prototype._getTooltip = function(b) {
		var c = a.jqChartDateFormat.masks, d = "";
		if (b.getSeconds() != 0)
	...
	s.prototype._getLabels = function() {
		...
		if (a == null || a.visible === false)
			return [];
		for (var j = this._getMaxOutsideTickMarksLength() + this.lblMargin, e = [], k = this
				._getMarkInterval(a, true), f = this._getLabelIntervals(k, a), d = 0; d < f.length; d++) {
			var g = f[d], i = this.getLabel(g), c = this._createLabel(i, a), l = this
					.getPosition(g), n = this.cy, m = l, h = b.rotatePointAt(m,
					n, -Math.PI / 2, this.cx, this.cy);
			c.x = h.x - j;
			c.y = h.y;
			c.textAlign = "right";
			e.push(c)
	...
	s.prototype._getTickMarks = function(a, k) {
		if (a == null || a.visible != true)
			return [];
		for (var h = [], s = this._getMarkInterval(a, k), t = a.length, j = this
				._getIntervals(s, a, k), g = this.sharedAxis, f = g
				._getIntervals(g.actualInterval), p, r, q, c, d = 0; d < j.length; d++) {
			var u = this.getPosition(j[d]);
			p = q = u;
			c = this.cy;
			r = c - t;
			for (var e = 0; e < f.length; e++) {
				var v = f[e], i = this.sharedAxis._getAngle(v), m = b
						.rotatePointAt(p, r, i, this.cx, this.cy), o = b
						.rotatePointAt(q, c, i, this.cx, this.cy), l = new n(
						m.x, m.y, o.x, o.y);
	...
	s.prototype._getGridLines = function(a, g) {
		if (a == null || a.visible != true)
			return [];
		for (var d = [], h = this._getMarkInterval(a, g), e = this
				._getIntervals(h, a, true), c = 0; c < e.length; c++) {
			var f = e[c], b = this._getRenderShape(f);
	...
	s.prototype._render = function(d) {
		...
		a.merge(d, f);
		var c = [], i = this._getTickMarks(this.minorTickMarks, false);
		a.merge(c, i);
	...
	s.prototype._getRenderShape = function(b) {
		var d = this.getPosition(b), a = d - this.origin + this.extent, c = this
				._createRenderShape(this.cx - a, this.cy - a, a);
		return c
	};
	s.prototype._createRenderShape = function(f, g, e) {
		...
		if (this.renderStyle != "polygon") {
			if (!b.isNull(a.startAngle) && !b.isNull(a.endAngle)) {
				var j = b.radians(a.startAngle) - Math.PI / 2, k = b
						.radians(a.endAngle)
						- Math.PI / 2;
		...
		for (var c = [], h = this.sharedAxis.actualMaximum, l = 2 * Math.PI / h, f = this.cx, g = this.cy
				- e, d = 0; d < h; d++) {
	...
	I.prototype._correctOrigin = function() {
		for (var a = 0, j = this.x, k = this.y, i = this.x + this.width, f = this.y
				+ this.height, g = this._getLabels(), c = 0; c < g.length; c++) {
			var b = g[c];
			if (b.x < j)
				a = Math.max(a, j - b.x);
			var e = b.x + b.width;
	...
	I.prototype._getAngle = function(b) {
		var d = this.actualMaximum, c = 2 * Math.PI / d, a = b * c;
	...
	I.prototype._getLabels = function() {
		...
		if (g == 0)
			return;
		for (var i = this.cx, j = this.cy, n = i, o = j - this.radius, m = 2
				* Math.PI / g, c = 8, h = [], f = this.labels, e = 0; e < g; e++) {
			var d = e * m, k = b.rotatePointAt(n, o, d, i, j), l = this
					.getLabel(e), a = this._createLabel(l, f);
			a.x = k.x;
			a.y = k.y;
			if (d == Math.PI) {
				a.x -= a.width / 2;
	...
	I.prototype._render = function(m) {
		var f = [], g = this.actualMaximum;
		if (g == 0)
			return;
		for (var d = this.cx, e = this.cy, q = d, s = e
				- this.sharedAxis.extent, r = d, t = e - this.radius, o = this.renderLinesOverGraph, p = 2
				* Math.PI / g, h = 0; h < g; h++) {
			var j = h * p, k = b.rotatePointAt(q, s, j, d, e), l = b
					.rotatePointAt(r, t, j, d, e), c = new n(k.x, k.y, l.x, l.y);
	...
	D.prototype._getAngle = function(g) {
		var e = this.actualMaximum - this.actualMinimum, f = this.endAngle
				- this.startAngle, d = b.radians(f), c = b
				.radians(this.startAngle), a;
		if (e === 0)
			a = c + d / 2;
		else
	...
	D.prototype._getLabels = function() {
		...
		if (d == null || d.visible === false)
			return [];
		for (var j = this.cx, k = this.cy, p = j + this.radius, q = k, e = 8, g = [], n = this
				._getMarkInterval(d, true), h = this._getLabelIntervals(n, d), o = this
				._getIntervalsLength(h), f = 0; f < o; f++) {
			var i = h[f], m = this.getLabel(i), c = this._getAngle(i), l = b
					.rotatePointAt(p, q, c, j, k), a = this._createLabel(m, d);
			a.x = l.x;
			a.y = l.y;
			c += Math.PI / 2;
			if (c == Math.PI) {
	...
	D.prototype._render = function(o) {
		var f = [], d = this.cx, e = this.cy, s = d, m = e;
		if (this.sharedAxis)
			m -= this.sharedAxis.extent;
		for (var t = d, u = e - this.radius, p = this.renderLinesOverGraph, j = this
				._getIntervals(this.actualInterval), q = this
				._getIntervalsLength(j), g = 0; g < q; g++) {
			var r = j[g], i = this._getAngle(r) + Math.PI / 2, k = b
					.rotatePointAt(s, m, i, d, e), l = b.rotatePointAt(t, u, i,
					d, e), c = new n(k.x, k.y, l.x, l.y);
			c.strokeStyle = this.strokeStyle;
			c.lineWidth = this.lineWidth;
			c.strokeDashArray = this.strokeDashArray;
			if (p)
